...
void msg_show_attr(int msg_id,struct msqid_ds msg_info)
{
	...
	if(ret == -1)
	{
		printf("fail to get message information\n");	
		return ;
	}
	printf("\n");
...
int main(int argc,char* argv[])
{
	...
	char *msgpath="/home/yifabao/ipc/msg/";
	key = ftok(msgpath,'b');
	...
	if(msg_id == -1)
	{
		printf("fail to build the message");	
		return 0;
	}
	printf("success to build the message\n");

	msg_show_attr(msg_id,msg_info);
	msg_sflags= IPC_NOWAIT;
	msg_mbuf.mtype = 10;
	memcpy(msg_mbuf.mtext,"test message gkh",sizeof("test message gkh"));
	ret = msgsnd(msg_id,&msg_mbuf,sizeof("test message gkh"),msg_sflags);
	if(ret == -1)
	{
	...
	if(ret == -1)
	{
		printf("fail to receive the message \n");	
	}
	else{
	...
	if(ret == -1)
	{
		printf("fail to set the message attribute\n");	
		return 0;
	}
	...
	if(ret == -1)
	{
		printf("fail to delete the message\n");	
		return 0;
	}
	return 0;
}
